...
public class HelperUtils {
	...
	public static final String approveResult = "approveResult"; // 审批结果
																// 0（同意），1（不同意）
	public static final String approveMsg = "approveMsg"; // 审批信息
	public static final String approveUser = "approveUser"; // 审批人
	public static final String date = "date"; // 审批日期 日期格式字符串,例如:2018-01-01
												// 00:00:00
	...
	public static final String time = "^((\\d{2}(([02468][048])|([13579][26]))[\\-\\/\\s]?((((0?[13578])|(1[02]))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])))))|(\\d{2}(([02468][1235679])|([13579][01345789]))[\\-\\/\\s]?((((0?[13578])|(1[02]))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\\-\\/\\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\\-\\/\\s]?((0?[1-9])|(1[0-9])|(2[0-8]))))))(\\s(((0?[0-9])|([1][0-9])|([2][0-4]))\\:([0-5]?[0-9])((\\s)|(\\:([0-5]?[0-9])))))?$";
	...
	public static String returnJsonMsg(String resultCode, String resultMsg) {
		...
		msg.append("{\"").append(RESULT_CODE).append("\":\"").append(resultCode).append("\",\"").append(RESULT_MSG)
	 ...
	 * @return
	 * 
	 * 		public static String exceptionMsg(JKBXHeaderVO bxVO, String spzt,
	 *         boolean isNull) { StringBuffer msg = new StringBuffer(); if
	 *         (isNull) {
	 *         msg.append("\"当前单据[").append(bxVO.getDjbh()).append("，").append(
	 *         bxVO.getPk_jkbx()).append("]状态标识为<").append(spzt)
	 *         .append(">,不再处理范围之类,请确认！\""); }
	 *         msg.append("\"当前单据[").append(bxVO.getDjbh()).append("，").append(
	 *         bxVO.getPk_jkbx()).append("]状态为<") .append((0 == bxVO.getSpzt() ?
	 *         "审批未通过" : "审批通过")).append(">请确认！\"");
	 * 
	 *         return msg.toString(); }
	 */
	...
	public static String isNullMsg(Map<String, Object> jsonMap) {
		StringBuffer msg = new StringBuffer();
		msg.append("\"当前单据[").append(jsonMap.get(billNo)).append("，").append(jsonMap.get(billId))
				.append("] 存在必填项没有填,请确认！\"");
	...
	public static String isApproverMsg(Map<String, Object> jsonMap) {
		StringBuffer msg = new StringBuffer();
		msg.append("\"当前单据[").append(jsonMap.get(billNo)).append("，").append(jsonMap.get(billId))
				.append("] 没有对应的审批人,接收到的信息[").append(jsonMap.get(approveUser)).append("],请确认！\"");
	...
	public static String isTimeMsg(Map<String, Object> jsonMap) {
		StringBuffer msg = new StringBuffer();
		msg.append("\"当前单据[").append(jsonMap.get(billNo)).append("，").append(jsonMap.get(billId))
				.append("] 时间格式不正确,接收到的信息[").append(jsonMap.get(date)).append("],请确认！\"");
	...
	public static String returnResMsg(Map<String, Object> jsonMap, Map<String, String> msgMap, boolean isApprover) {
		...
		if (RESULTCODE_FAIL.equals(msgMap.get(RESULT_CODE))) {
			msg.append("\"当前单据[").append(jsonMap.get(billNo)).append("，").append(jsonMap.get(billId))
					.append("] 处理失败，失败原因[").append(msgMap.get(RESULT_MSG)).append("],请确认！\"");
		} else {
			if (isApprover) {
